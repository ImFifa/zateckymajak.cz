...
/**
 * @property-read \App\Model\ArticleModel $article
...
class ProjectModelRepository extends ModelRepository
{
	...
	{
		return $this->article->getTable()->where('public', 1)->where('lang', $lang)->where('slug', $slug)->fetch();
	}

	public function getPublicNews(string $lang): Selection
	{
		return $this->article->getTable()->where('public', 1)->where('lang', $lang)->order('created DESC')->order('id DESC');
	}
	...
	{
		return $this->article->getTable()->where('public', 1)->where('lang', $lang)->where('category_id', $category_id)->order('created DESC')->limit($limit, $offset);
	}
